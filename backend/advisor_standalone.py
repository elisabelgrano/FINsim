"""
FINsim Virtual Advisor API - Standalone FastAPI Server
...
Usage:
    ...
    uvicorn advisor_standalone:app --host 0.0.0.0 --port 8000
...
logger = logging.getLogger("finsim.advisor")
...
Provide your response as a JSON object with tactical guidance and recommended charts."""
        ...
        logger.info(f"Calling Ollama with model {cls.MODEL_NAME}")
...
async def lifespan(app: FastAPI):
    ...
    logger.info("=" * 60)
    logger.info("FINsim Virtual Advisor API starting...")
    logger.info(f"Ollama endpoint: {OllamaAdvisor.OLLAMA_GENERATE_URL}")
    logger.info(f"Ollama model: {OllamaAdvisor.MODEL_NAME}")
    logger.info(f"Timeout: {OllamaAdvisor.OLLAMA_TIMEOUT}s")
    logger.info("=" * 60)
    yield
...
async def chat_with_advisor(request: AdvisorRequest) -> AdvisorResponse:
    ...
    **Example Response:**
    ...
    ```
    """
    user_msg_preview = (request.user_message[:50] if request.user_message else '(empty)')
    logger.info(f"Advisor request: user_message='{user_msg_preview}...'")
...
async def get_docs() -> dict:
    ...
    return {
        ...
        "version": "1.0.0",
        "endpoints": {
            "POST /api/advisor/chat": "Generate tactical advice from metrics",
            "GET /health": "Health check",
            "GET /api/advisor/charts": "List available chart codes",
        ...
        "usage_example": {
            "request": {
                ...
                "user_message": "Which segments need attention?"
            },
            "response": {
                "suggerimento_breve": "Focus engagement on high-value clients in equity segment",
                "dettaglio_risposta": "Performance analysis shows...",
                "grafici_consigliati": ["HEATMAP_PERFORMANCE", "BAR_PRODOTTI"]
            }
        }
    }
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            response = requests.get("http://10.12.7.53:11434/api/tags", timeout=5)
            logger.info(f"Ollama health check passed")
        except Exception as e:
            logger.warning(f"Ollama may not be reachable: {e}")
            logger.warning("The API will start but advisor calls may fail")
